...
def initialize(dictionary=None):
    ...
    with DICT_LOCK:
        ...
        if abs_path == os.path.join(_curpath, "dict.txt"):
            cache_file = os.path.join(tmp_dir if tmp_dir else tempfile.gettempdir(),"jiebazhc.cache")
        else:  # custom dictionary
            cache_file = os.path.join(tmp_dir if tmp_dir else tempfile.gettempdir(),"jiebazhc.u%s.cache" % md5(
