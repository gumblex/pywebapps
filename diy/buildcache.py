...
ccount = cur.execute(
    "SELECT count(id) FROM sentences WHERE type = 0").fetchone()[0]
mcount = cur.execute(
    "SELECT count(id) FROM sentences WHERE type = 1").fetchone()[0]
print(count, ccount, mcount)
cache.gc()
print('GC done.')
...
if ccount > mcount:
    cur.execute(
        "DELETE FROM sentences WHERE id IN (SELECT id FROM sentences WHERE type = 0 ORDER BY RANDOM() LIMIT ?)", (ccount - mcount,))
else:
    cur.execute(
        "DELETE FROM sentences WHERE id IN (SELECT id FROM sentences WHERE type = 1 ORDER BY RANDOM() LIMIT ?)", (mcount - ccount,))
