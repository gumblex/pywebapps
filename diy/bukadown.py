...
def bukadownloader(comicid, chapid):
    ...
    """
    postdata = ('i=%s&z=1&p=android&v=9&c=91643f635a86aad35b9f942db576f233' % urllib.parse.quote(urllib.parse.quote(
        json.dumps({"f": "func_getdownurl3", "ver": 3, "mid": comicid, "cid": chapid, "restype": 2})))).encode('utf-8')
    req = urllib.request.Request("http://cs.bukamanhua.com:8000/request.php?t=%d" % time.time(
    ), data=postdata, headers={"Content-Type": "application/x-www-form-urlencoded"})
    downlist = json.loads(
        gzip.decompress(urllib.request.urlopen(req).read()).decode('utf-8'))
    if downlist['ret']:
        return None
